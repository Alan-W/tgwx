...
var $	 = require('jquery');
var WeixinActions = require('../actions/WeixinActions.js');
...
var WeixinStore = Reflux.createStore({
	listenables: [WeixinActions], // 监听actions
	isReady: false,
	...
	onSetup: function(callbk,extra_apis, isForbid){
		var me = this;
		...
		$.ajax({
			...
			success:function(resp){
				...
				if (isForbid) {
					console.log(' is Frobid -- --- ', isForbid);
					me.forbidSharing();
				}
				me.isReady = true;
				me.trigger(me.isReady);
	...
	forbidSharing: function(privs){
		if (privs && privs == 1)
			return;
		console.log('forbid sharing');
		// alert('forbid share!');
		wx.hideAllNonBaseMenuItem();
	},
	enableSharing: function(istrue){
